...
?>
	...
	<div class=main>
		...
		<a href="creerProduit.php">Créer un nouveau produit</a>
		<br><br>
		<table>
		<tr>
		<th>ID</th><th>Nom</th><th>Type</th><th>Degré</th><th>Prix</th><th>Image</th>
		</tr>
		<?php
		try {
			$reqCat=$bdd->prepare("SELECT * FROM produit");
			$reqCat->execute();
			while ($result=$reqCat->fetch(PDO::FETCH_OBJ) ) {  
				echo '<tr>';
				
				echo '<td>['.$result->ID.']</td><td>'.$result->nom.'</td><td>'.$result->type.'</td><td>'.$result->pourcentage.'</td><td>'.$result->prix.'</td><td>'.$result->image.'</td>' ;
				echo '<td><table><tr>';
				echo '<td><form id="form'.$result->ID.'-mod" action="editProduit.php" method="post"> <input type=hidden value="'.$result->ID.'" name="ID"><input type=hidden value="'.$result->ID.'" name="submitted"><a href="#" onclick="document.getElementById(\'form'.$result->ID.'-mod\').submit();">&nbsp;Modifier&nbsp;</form></td>';
				echo '<td><form id="form'.$result->ID.'-supp" action="suppProduit.php" method="post"> <input type=hidden value="'.$result->ID.'" name="ID"><input type=hidden value="'.$result->ID.'" name="submitted"><a href="#" onclick="document.getElementById(\'form'.$result->ID.'-supp\').submit();">&nbsp;Supprimer&nbsp;</form></td>';
				echo '</tr></table></td>'; 
				echo '</tr>';
				} 
			$reqCat->closeCursor(); 
